...
- 可创建服务器根文件系统 tar.gz 在线归档快照，写入 OpenClaw-Debugger-Private\OpenClaw-Server-Backup 并生成含 SHA-256 的清单。备份前先用同一 tar/gzip 流预估压缩总量，再传输并显示已传输大小、平均速度和剩余时间；预估会完整扫描、压缩一次，但不在服务器留下临时归档，文件在线变化时总量可能略有偏差。
...
桌面快捷方式 OpenClaw-Debugger.lnk 当前指向 bin\WeightBuild-BackupProgress\net10.0-windows\OpenClawDebugger.exe；当前运行的旧窗口不会热更新，关闭后再次从快捷方式启动会加载新版本。若终端中的 ssh admin@106.14.173.90 在同一 Windows 用户下可用，应用沿用当前 OpenSSH 身份，不复制或要求输入 SSH 私钥。
