...
- 导航栏右侧的独立进度模块展示估算总量、已传输大小、平均传输速度、预计剩余时间和图形化进度条；完成后生成归档和 `snapshot-manifest.json`，并将临时目录改名为带时间戳的备份目录。
- SSH 配置了保活检测。估算或传输中遇到可恢复的网络中断时最多自动重试 8 次，按 2、4、8、16、30 秒逐步等待；传输重试会从头生成并传输 gzip 流，清空本轮部分文件，不拼接不同时间点的压缩流。多次失败后清理临时文件并在进度模块显示原因。
...
- 最近一次已完成构建：`RetryProgress` 配置输出到 `bin\RetryProgress\net10.0-windows`，构建成功，0 个警告、0 个错误；本地 JavaScript 语法检查通过。没有因此执行服务器备份或恢复测试。
- 桌面快捷方式 `OpenClaw-Debugger.lnk` 已指向 `bin\RetryProgress\net10.0-windows\OpenClawDebugger.exe`。已运行的旧窗口不会热更新；关闭后从桌面快捷方式重新启动即可加载新版。
